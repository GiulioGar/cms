function allinea_r(colonne, margine, margine_larghezza){
	...
	if (!elemento_img.length){soloitem=1; altezza=149;}
    for (i=0;i<50;i++) {
	...
	$('.ranking-option:nth-child('+i+')').css({left: larghezza*(conta_elementi_riga-1),top:-(altezza*(count-1))+riga*(altezza+mar)-riga*(altezzaitm), position:'relative'});
	if (conta_elementi_riga==imposta_colonne){conta_elementi_riga=0; riga++}
    }
    if (conta_elementi_riga==0){riga--;}
    $('div#console').css({top:-(altezza*(count-2))+riga*(altezza+mar)-riga*(altezzaitm), position:'relative'});
