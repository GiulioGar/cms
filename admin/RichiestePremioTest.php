...
$add_euro = mysqli_query($query_aggiorna, $admin) or die(mysql_error());
	...
	$query_conta = "SELECT COUNT(user_id) as tot FROM t_history_test where event_type='withdraw'";
	...
	$surClo2 = mysqli_query($query_conta2, $admin) or die(mysql_error());
...
{
//COPIO HISTORY
$query_copia_history_copy="INSERT t_history_test (user_id, event_date, event_type, event_info, prev_level, new_level)
SELECT user_id, event_date, event_type, event_info, prev_level, new_level
FROM t_user_history where event_type='withdraw' and user_id NOT IN (SELECT user_id FROM t_history_test where t_history_test.event_date=t_user_history.event_date)";
...
$query_cerca = "SELECT * FROM t_history_test,t_user_info where pagato like '$cerca_progetto' AND t_history_test.user_id=t_user_info.user_id order by event_date desc";
//$query_cerca = "SELECT * FROM t_user_history where event_type='withdraw' and user_id NOT IN (SELECT user_id FROM t_history_test where event_type='withdraw')";
...
<div class="content-wrapper">
 ...
 <form role="form" name="modulo_cerca_prj" action="RichiestePremioTest.php" method="get">
...
<form  action="RichiestePremioTest.php" method="post">
		 ...
		 <td style='background:".$bacCol."'>".$euroPaga."</td><td>".$row['prev_level']."</td><td>".$row['new_level']."</td><td>".$newdate."</td>";
		  if ($row['pagato']==0){echo "<td colspan='2'>Non assegnato</td></tr>";}
			...
			if ($code!="")	
				...
				mysqli_select_db($database_admin, $admin);
				$query_aggiorna = "UPDATE t_history_test SET pagato=1, codice='$code', giorno_paga='$data' WHERE pagato=0 and id='".$row['id']."'";
...
<?php
$query_pago= "SELECT COUNT(*) as total FROM millebytesdb.t_history_test where event_info='Buono regalo da 2 Euro' and event_date> '2017-03-01'";
$t_PAGO = mysqli_query($query_pago, $admin) or die(mysql_error());
$data2=mysqli_fetch_assoc($t_PAGO);

$query_pago= "SELECT COUNT(*) as total FROM millebytesdb.t_history_test where event_info='Buono regalo da 5 Euro' and event_date> '2017-03-01'";
$t_PAGO = mysqli_query($query_pago, $admin) or die(mysql_error());
$data5=mysqli_fetch_assoc($t_PAGO);

$query_pago= "SELECT COUNT(*) as total FROM millebytesdb.t_history_test where event_info='Buono regalo da 10 Euro' and event_date> '2017-03-01'";
$t_PAGO = mysqli_query($query_pago, $admin) or die(mysql_error());
$data10=mysqli_fetch_assoc($t_PAGO);

$query_pago= "SELECT COUNT(*) as total FROM millebytesdb.t_history_test where event_info='Buono regalo da 20 Euro'  and event_date> '2017-03-01'";
$t_PAGO = mysqli_query($query_pago, $admin) or die(mysql_error());
$data20=mysqli_fetch_assoc($t_PAGO);

$query_bud= "SELECT * FROM millebytesdb.cassa_buoni where type='euro2'";
$t_bud = mysqli_query($query_bud, $admin) or die(mysql_error());
...
<table style="font-size:11px"  class="table table-striped table-bordered">
		<tr class='intesta'><th>&nbsp;</th><th colspan="" >&nbsp;</th></tr>
		...
		<tr class=''>
		<td>Budget</td><td><?php echo $bud2['num']; ?></td><td><?php echo $bud5['num']; ?></td><td><?php echo $bud10['num']; ?></td><td><?php echo $bud20['num']; ?></td>
		</tr>
		<tr class=''>
		<td>Pagati</td><td><?php echo $data2['total']; ?></td><td><?php echo $data5['total']; ?></td><td><?php echo $data10['total']; ?></td><td><?php echo $data20['total']; ?></td>
		</tr>
		<tr class=''>
		<td>Giacenza</td><td><?php echo $gia2; ?></td><td><?php echo $gia5; ?></td><td><?php echo $gia10; ?></td><td><?php echo $gia20; ?></td>
		</tr>
		<tr class=''>
		<td><b>Mensili</b></td><td><b><?php echo number_format($giaM2,0); ?></b></td><td><b><?php echo number_format($giaM5,0); ?></b></td><td><b><?php echo number_format($giaM10,0); ?></b></td><td><b><?php echo number_format($giaM20,0); ?></b></td>
		</tr>
</table>
</div>
</div>
