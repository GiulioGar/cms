...
$query_last="SELECT v.user_id as user,u.user_id, u.email as email, v.code as code, v.concorso FROM t_user_win as v, t_user_info as u WHERE v.concorso=$ccs AND u.user_id=v.user_id ORDER BY premio ASC LIMIT 5";
mysqli_select_db($database_admin, $admin);
...
$query_rand = "SELECT u.user_id as user , u.email as email, u.first_name as name, u.province_id as proid, u.address as address, u.city as city,u.code as cap, u.second_name as name2, t.code as code, t.received_on as buy, t.id as id_tic, t.user_id as usertic, t.valid  FROM t_virtual_tickets as t, t_concorso as c, t_user_info as u WHERE address<>'' AND c.status_v=1 AND t.received_on <= c.end_date AND t.received_on >= c.start_date AND u.user_id=t.user_id AND t.valid='1' ORDER BY RAND() LIMIT 0,$np";
mysqli_select_db($database_admin, $admin);
